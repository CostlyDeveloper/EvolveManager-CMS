...
// NOTIFICATIONS
function showMessage(type, title, content, new_delay) {
    var opts = {
        title: "Over Here",
    ...
    switch (type) {
        case 'error':
            if (title) { //Custom title
                ...
                opts.title = lang.messageErrorTitle;
            }
            if (content) { //custom content
                opts.text = content;
            } else {
                opts.text = lang.messageError;
            }
            opts.type = "error";
            opts.addclass = "alert-danger";
        ...
        case 'success':
            if (title) { //Custom title
                ...
                opts.title = lang.messageSuccess;
            }
            if (content) { //custom content
                opts.text = content;
            } else {
                opts.text = lang.messageCompleted;
            }
            opts.type = "success";
            opts.addclass = "alert-success";
            opts.styling = 'bootstrap3';
            break;
    }
    new PNotify(opts);
...
$(window).load(function () {
    $(".overlay_loader").fadeOut("slow");
});
jQuery(document).ready(function ($) {
    //REMOVE SIDEBAR ON SMALL SCREEN
    var eventFired = 0;
    if ($(window).width() < 750) {
        $(document.body).delay(1500).addClass('nav-md');
    } else {
        eventFired = 1;
        $(document.body).delay(1500).addClass('nav-sm');
    }
    $('.left_menu').delay(1500).removeClass('hidden');
    $(window).on('resize', function () {
        if (!eventFired) {
            if ($(window).width() < 960) {
                $(document.body).removeClass('nav-sm');
                $(document.body).addClass('nav-md');
            } else {
                $(document.body).removeClass('nav-md');
                $(document.body).addClass('nav-sm');
            }
        }
    });
    // /REMOVE SIDEBAR ON SMALL SCREEN
    lazyload();
    //SORTABLE
    $.fn.serializeTree = function (attribute, levelString, exclude) {
        var dataString = '';
        var elems;
...
$(document).ready(function () {
    ...
    if ($(".featured_switch")[0]) {
        var elems = Array.prototype.slice.call(document.querySelectorAll('.featured_switch'));
        elems.forEach(function (html) {
            var switchery = new Switchery(html, {
                color: '#f6911d'
            });
...
function init_logout() {
    // PROMOTE SWITCH (list)
    $(document).on('click touchstart', '.logout_btn', function (e) {
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: 'system/logout/actions/logout.php',
            success: function () {
                showMessage('success', '', '');
                location.reload();
            },
