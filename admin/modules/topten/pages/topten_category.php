<?php 
  ...
  $get_category = $mysqli->query("
    SELECT evolve_topten_category.*, evolve_topten_category_data.*, evolve_topten_category.id as wcat_id
    ...
    WHERE evolve_topten_category.id = '$instance_id'
  ");
  if($developing) if(!$get_category) print_r($mysqli->error);
  $get_layout = $mysqli->query("
    SELECT evolve_topten_category_type.id, evolve_topten_category_type.name
    FROM evolve_topten_category_type
    ORDER BY id
  ");
                  ...
                  <?php $slugs_arr = languages();
                    ...
                    foreach($slugs_arr as $slug){
                      ...
                      $get_wcat = $mysqli->query("
                        SELECT evolve_topten_category_data.seo_id, evolve_topten_category_data.description, evolve_topten_category_data.title, evolve_topten_category.id as wcat_id
                        FROM evolve_topten_category
                           
                        LEFT JOIN evolve_topten_category_data
                        ON evolve_topten_category_data.for_instance = evolve_topten_category.id
                          AND evolve_topten_category_data.lang =  '$slug'
