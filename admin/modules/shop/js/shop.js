...
function initEditCategory(_ModulePath, _ItemName) {
    $(document).on('submit', '#edit_' + _ItemName +'_category_form', function () {
        var formDetails = $('#edit_' + _ItemName + '_category_form, #token_data');
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: _ModulePath + 'actions/edit_' + _ItemName + '_category.php',
            data: formDetails.serialize(),
            success: function (data) {
                $.each(data, function (key, value) {
                    $('input[name=' + key + ']').val(value);
...
jQuery(document).ready(function ($) {
    ...
    initEditCategory(modulePath, itemName);
